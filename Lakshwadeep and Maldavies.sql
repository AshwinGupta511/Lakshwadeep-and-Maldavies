...
select t.NumberofVisitors from Destination d join TouristArrivals as t
on d.DestinationID = t.DestinationID
where Year = 2019 and d.Name = 'Maldives'

--2)Retrieve the average budget per day and best time to visit for destinations with more than 10,000 tourists in 2022?
select d.AverageBudgetPerDay, I.Year , d.Name , d.BestTimeToVisit from   Destination d join IndianTourists I
on d.DestinationID = I.DestinationID and I.NumberofTourists > 1000 and year = 2022

--3)Retrieve the unique features of tourist attractions in Lakshadweep?
select TouristAttractions.name,Description from TouristAttractions
join 
Destination on TouristAttractions.DestinationID=Destination.DestinationID where Destination.name='Lakshadweep'

--4)Retrieve the number of Indian tourists who visited the Maldives in 2021?

select it.NumberofTourists from Destination as d join 
IndianTourists as it on d.DestinationID = it.DestinationID
where 
Name = 'Maldives' and Year = 2021

--5)Find the destinations with the highest tourism revenue in 2022?
select top 1 (NumberofVisitors*AverageBudgetPerDay) as TourRevenue, d.Name  from Destination as d join 
TouristArrivals as ta
on d.DestinationID = ta.DestinationID
where year = 2022
